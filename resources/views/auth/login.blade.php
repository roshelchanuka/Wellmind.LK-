...
@section('title', 'Login - WellMind LK')
@section('content')
...
<form class="form" method="POST" action="{{ route('login') }}">
    @csrf
    <h2 class="auth-form-header" data-key="loginHeader">Welcome Back</h2>
    ...
    @if(session('success'))
        <div class="success-msg-auth">
            {{ session('success') }}
        </div>
    @endif

    <div class="flex-column">
      <label data-key="emailLabel">Email</label>
    </div>
    <div class="inputForm">
        <svg height="20" viewBox="0 0 32 32" width="20" xmlns="http://www.w3.org/2000/svg"><g id="Layer_3" data-name="Layer 3"><path d="m30.853 13.87a15 15 0 0 0 -29.729 4.082 15.1 15.1 0 0 0 12.876 12.918 15.6 15.6 0 0 0 2.016.13 14.85 14.85 0 0 0 7.715-2.145 1 1 0 1 0 -1.031-1.711 13.007 13.007 0 1 1 5.458-6.529 2.149 2.149 0 0 1 -4.158-.759v-10.856a1 1 0 0 0 -2 0v1.726a8 8 0 1 0 .2 10.325 4.135 4.135 0 0 0 7.83.274 15.2 15.2 0 0 0 .823-7.455zm-14.853 8.13a6 6 0 1 1 6-6 6.006 6.006 0 0 1 -6 6z"></path></g></svg>
        <input type="email" name="email" class="input" placeholder="Enter your Email" data-placeholder-key="emailPlaceholder" value="{{ old('email') }}" required>
    </div>
    ...
    <div class="inputForm">
        <svg height="20" viewBox="-64 0 512 512" width="20" xmlns="http://www.w3.org/2000/svg"><path d="m336 512h-288c-26.453125 0-48-21.523438-48-48v-224c0-26.476562 21.546875-48 48-48h288c26.453125 0 48 21.523438 48 48v224c0 26.476562-21.546875 48-48 48zm-288-288c-8.8125 0-16 7.167969-16 16v224c0 8.832031 7.1875 16 16 16h288c8.8125 0 16-7.167969 16-16v-224c0-8.832031-7.167969-16-16-16zm0 0"></path><path d="m304 224c-8.832031 0-16-7.167969-16-16v-80c0-52.929688-43.070312-96-96-96s-96 43.070312-96 96v80c0 8.832031-7.167969 16-16 16s-16-7.167969-16-16v-80c0-70.59335 57.40625-128 128-128s128 57.40625 128 128v80c0 8.832031-7.167969 16-16 16zm0 0"></path></svg>        
        <input type="password" name="password" class="input" placeholder="Enter your Password" data-placeholder-key="passwordPlaceholder" required>
    </div>
    
    <div class="flex-row">
      <div style="display: flex; align-items: center; gap: 5px;">
        <input type="checkbox" name="remember" id="remember">
        <label for="remember" class="form-item-label" data-key="rememberMe">Remember me</label>
      </div>
      <a href="{{ url('/forgot-password') }}" class="span" data-key="forgotPassword">Forgot password?</a>
    </div>
    
    <button type="submit" class="button-submit" data-key="signIn">Sign In</button>
    <p class="p"><span data-key="noAccount">Don't have an account?</span> <a href="{{ route('register') }}" class="span" data-key="registerNow">Sign Up</a></p>
    
    <p class="p line" data-key="orWith">Or With</p>
    ...
    <div class="flex-row">
      <button type="button" class="btn google">
        <svg version="1.1" width="20" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
          ...
          <path style="fill:#518EF8;" d="M507.527,208.176C510.467,223.662,512,239.655,512,256c0,18.328-1.927,36.206-5.598,53.451c-12.462,58.683-45.025,109.925-90.134,146.187l-0.014-0.014l-73.044-3.727l-10.338-64.535c29.932-17.554,53.324-45.025,65.646-77.911h-136.89V208.176h138.887L507.527,208.176h138.887L507.527,208.176L507.527,208.176z"></path>
